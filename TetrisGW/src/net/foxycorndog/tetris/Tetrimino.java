...
/**
 * File: Tetrimino.java Author: Henry Thomas Rybolt Programming: 3rd Hour Last
 * Modified: May 06, 2013 Description: A Tetrimino can rotate and move in the
 * world it was created by moving its four squares in the world
 */
public class Tetrimino
{
	...
	public void Tetrimino(int[][] shape, Color c, int x, int y, World w)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			Location l = convertLFromCToG(new Location(x + shape[i][0], y
					+ shape[i][1]));
			// w.add(l, squares[i]);
		}
		this.w = w;
	...
	/**
	 * creates a tetrimino at the top of the world w, a color that cooresponds
	...
	public void updateLocations()
	{
		for (int i = 0; i < 4; i++)
		{
			Location next = convertLFromCToG(new Location(shape[i][0] + x,
					shape[i][1] + y));
			Square s = squares[i];
			if (s.getGrid() != null)
				s.moveTo(next);
	...
	/**
	 * converts a location from the cartesian grid(standard x and y axis) to
	 * GridWorlds's grid(with rows and columns)
	 */
	...
	public void moveLeft()
	{
		...
		{
			Square s = squares[i];
			Location next = convertLFromCToG(new Location(shape[i][0] + x - 1,
					shape[i][1] + y));
			if (spaceIsFree(s, next) == false)
				ableToMove = false;
		}
